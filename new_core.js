(function () {
    ...
    function initGUI() {
        ...
        container.innerHTML = `
            ...
            <input id="answer" class="pref-input" type="text" placeholder="例：A,B,C,YES" style="width:100%;margin:0;font-size:16px;padding:6px;box-sizing:border-box;background:#eef7ff;border:1px solid #06f;color:#000;">
    ...
    function handleWarningPopups() {
        const warnings = ["票券都被選走了", "如果其他用戶沒有完成報名", "不要重新整理頁面", "請稍後再試", "沒有可以購買", "單一分頁", "系統繁忙"];
    ...
    function handleTicketingPage() {
        ...
        if (selectTicket()) {
            ...
            setTimeout(() => {
                ...
                const answerInputs = Array.from(document.querySelectorAll('input[type="text"]')).filter(el => {
                    return !el.closest('#kenny-panel') && !el.classList.contains('member-code') && !(el.placeholder && el.placeholder.includes('會員')) && !el.closest('.ticket-quantity');
                });
                ...
                if (ansRaw) {
                    ...
                    // 2. 處理選擇題 (Radio + Checkbox)，避開同意條款的 Checkbox
                    const choiceInputs = document.querySelectorAll('input[type="radio"], input[type="checkbox"]:not(#person_agree_terms):not([id*="agree"])');
                    ...
                    if (answerInputs.length > 0 && answerInputs[0].value === "") answerInputs[0].focus();
                }
                ...
                const memField = document.querySelector('input.member-code, input[placeholder*="會員"]');
                if (mem && memField) {
                    memField.value = mem;
                    memField.dispatchEvent(new Event("input", { bubbles: true }));
                    memField.dispatchEvent(new Event("change", { bubbles: true }));
                }

                // 點擊下一步
                const next = [...document.querySelectorAll('button,input')].find(b => !b.disabled && ((b.innerText || b.value || "").includes("下一步") || (b.innerText || b.value || "").includes("配位")));
                if (next) { next.click(); }
            }, 200);
        } else {
            const now = Date.now();
